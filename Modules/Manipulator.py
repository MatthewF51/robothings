...
COMMANDS = {
    "Move Arm Forward": {
        "function": lambda distance: send_command(
            "rosservice call /arm_teleop/move_rel",
            f"x: {distance} y: 0.0 z: 0.0"
        ),
    ...
    "Rotate Wrist Clockwise": {
        "function": lambda angle: send_command(
            "rosservice call /arm_teleop/rotate_wrist",
            f"angle: {angle}"
        ),
    ...
    "Release (Open Hand)": {
        "function": lambda: send_command(
            "rosservice call /gripper_controller/release",
            ""
        ),
        "inputs": {},
    },
